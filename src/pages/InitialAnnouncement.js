...
export default class InitialAnnouncement extends React.Component {
  constructor(props) {
    super(props)
  ...
  onInputTextHandler = (text) => {
    ...
    if(this.state.input === '') {
      // item.map(items => this.setState({article: {...this.getData()}, items}));
      this.setState({filterdata: this.state.announcement});
    }
  }
